...
{
    Exploit exploit;
    if (exploit.setFirmwareVersion(fw))
        cleanup(1);
    if (exploit.setInterface(interface))
        cleanup(1);
...
int main(int argc, char *argv[])
{
    ...
    auto cli = (("network interface" % required("--interface") & value("interface", interface),
